...
export default function PMLayout({ children }) {
  ...
  const menuItems = [
    { name: 'PM Dashboard', href: '/pm', icon: '🏢' },
    { name: 'Project Status', href: '/pm/projects', icon: '📊' },
    { name: 'All Requests', href: '/pm/all-requests', icon: '📁' },
  ];
    ...
    <div className="min-h-screen flex bg-white dark:bg-[#121212] text-gray-900 dark:text-gray-100 transition-colors duration-300">
      ...
      `}>
        <div className="p-4 flex items-center justify-between border-b border-inherit">
          {isSidebarOpen && <span className="font-bold text-lg text-blue-600">E-PROC PM</span>}
          ...
          {menuItems.map((item) => (
            <Link key={item.name} href={item.href}>
              <div className={`flex items-center gap-4 p-3 rounded-lg cursor-pointer transition-colors ${
                pathname === item.href ? 'bg-blue-600 text-white' : 'hover:bg-gray-500/10'
              }`}>
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-20'}`}>
        <header className="h-16 flex items-center justify-between px-8 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1a1a1a]">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-500">PM Workspace (Final Approval)</h2>
          <div className="flex items-center gap-3 font-bold">Project Manager</div>
        </header>
        ...
        <footer className="py-4 text-center text-xs text-gray-400">
            © {currentYear} Elsicom Engineering - Project Management Office.
          </footer>
      </div>
    </div>
  );
