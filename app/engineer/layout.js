...
export default function EngineerLayout({ children }) {
  ...
  // Memastikan class 'dark' menempel pada tag <html> atau <body>
  useEffect(() => {
    if (isDarkMode) {
  ...
  const menuItems = [
    { name: 'Dashboard', href: '/engineer', icon: '🏠' },
    { name: 'Material Request', href: '/engineer/material-request', icon: '📋' },
    { name: 'Warehouse Release', href: '/engineer/warehouse-release', icon: '📦' },
    { name: 'Fault Report', href: '/engineer/fault-report', icon: '⚠️' },
  ];
    ...
    <div className="min-h-screen flex bg-white dark:bg-[#121212] text-gray-900 dark:text-gray-100 transition-colors duration-300">
      ...
      `}>
        <div className="p-4 flex items-center justify-between border-b border-inherit">
          {isSidebarOpen && <span className="font-bold text-lg">E-PROC</span>}
          ...
          {menuItems.map((item) => (
            <Link key={item.name} href={item.href}>
              <div className={`flex items-center gap-4 p-3 rounded-lg cursor-pointer transition-colors ${
                pathname === item.href ? 'bg-red-600 text-white' : 'hover:bg-gray-500/10'
              }`}>
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-20'}`}>
        <header className="h-16 flex items-center justify-between px-8 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1a1a1a]">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-500">Engineer Workspace</h2>
          <div className="flex items-center gap-3 font-bold">Engineer User</div>
        </header>
        ...
        <footer className="py-4 text-center text-xs text-gray-400">
            © {currentYear} Elsicom Engineering.
          </footer>
      </div>
    </div>
  );
